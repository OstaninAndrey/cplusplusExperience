/**
 * Created by Shovkoplyas on 23.12.2015.
 */
...
public class Validate {
    ...
    public void run() {
        ...
        int y = inf.nextInt();
        inf.nextLine();
        ensureLimits(n, 2, MAX_SIZE, String.format("Incorrect n = %d", n));
        ensureLimits(x, 1, MAX_NUMBER, String.format("Incorrect x = %d", x));
        ensureLimits(y, 1, x, String.format("Incorrect y = %d", y));
        for (int i = 0; i < n; i++) {
            int a = inf.nextInt();
            ensureLimits(a, 1, MAX_NUMBER, String.format("Incorrect a[%d] = %d", i + 1, a));
        }
        inf.nextLine();
        inf.close();
    }
